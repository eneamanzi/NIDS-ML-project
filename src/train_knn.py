"""
Training k-NN per CICIoT2023 - Multi-Class Classification.
Rifattorizzato per supportare dataset original e SMOTE con path management robusto.

Usage:
    python src/train_knn.py --dataset-type original
    python src/train_knn.py --dataset-type smote --n-neighbors 7
"""
...
MODELS_DIR = BASE_DIR / "models" / "kNN"
DOCS_DIR = BASE_DIR / "docs" / "kNN"
...
def get_output_paths(dataset_type):
    ...
    return {
        'model': model_dir / f'knn_model_{dataset_type}.pkl',
        'plots': plots_dir
    }
...
def train_knn(X_train, y_train, n_neighbors=5):
    ...
    Returns:
        Trained model
    """
    print_header("TRAINING k-NN")
    
    print(f"Hyperparameters:")
    print(f"  n_neighbors: {n_neighbors}")
    print(f"  weights: distance (weighted by inverse distance)")
    print(f"  algorithm: auto")
    print(f"  metric: minkowski (p=2, equivalent to euclidean)")
    print(f"  n_jobs: -1 (use all CPU cores)")
    
    print("\n⚠️ WARNING: k-NN training stores all training data")
    print(f"   Memory requirement: ~{X_train.nbytes / 1024**2:.1f} MB")
    ...
    print("   (k-NN is instance-based: 'training' = storing data)")
...
def evaluate_model(model, X_train, X_test, X_val, y_train, y_test, y_val,
    ...
    print("⏳ Making predictions (k-NN is slow on large datasets)...")
    ...
    # Note: k-NN tipicamente ha alta accuracy sul training (memorization)
    acc_diff = metrics['Train']['accuracy'] - metrics['Test']['accuracy']
    if acc_diff > 0.05:
        print(f"\n⚠️ Train accuracy molto più alta del test (diff: {acc_diff:.4f})")
        print("   Questo è normale per k-NN (memorization effect)")
    ...
    ax.bar(x, test_values, width, label='Test', alpha=0.8, color='purple')
    ax.bar(x + width, val_values, width, label='Val', alpha=0.8, color='orange')
    
    ax.set_ylabel('Score', fontsize=12)
    ax.set_title(f'Metrics Comparison - k-NN\nDataset: {dataset_type.upper()}',
                 fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ...
    ax.bar(x, per_class_recall, width, label='Recall', alpha=0.8, color='purple')
    ax.bar(x + width, per_class_f1, width, label='F1-Score', alpha=0.8, color='orange')
    
    ax.set_ylabel('Score', fontsize=12)
    ax.set_title(f'Per-Class Performance - k-NN\nDataset: {dataset_type.upper()}',
                 fontsize=14, fontweight='bold')
    ax.set_xticks(x)
...
def save_model(model, output_paths, dataset_type):
    ...
    print("\n⚠️ NOTE: k-NN model contains all training data")
    print("   Large training set = large model file")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Train k-NN on CICIoT2023 (Original or SMOTE)'
    )
    ...
    print("\n" + "🎯"*40)
    print(f"k-NN TRAINING - {args.dataset_type.upper()}".center(80))
    print("🎯"*40)
    ...
    model = train_knn(X_train, y_train, n_neighbors=args.n_neighbors)
    ...
    if meets_requirements:
        ...
        print("\n💡 Tips for improving k-NN:")
        print("   - Try different n_neighbors values")
        print("   - k-NN works better on SMOTE-balanced dataset")
        print("   - Consider feature selection (reduce dimensionality)")
