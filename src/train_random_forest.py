"""
Training Random Forest - PARQUET OPTIMIZED + PATH INDEPENDENT

OTTIMIZZAZIONI v2.0:
- INPUT: .parquet (invece di .pkl)
- PATH: Indipendente da directory invocazione
- MEMORY: float32 optimization
- PARALLELIZATION: n_jobs=-1 per tutti i core
...
def plot_feature_importance(model, feature_names, output_paths, dataset_type, n_features=20):
    ...
    print(f"\nTop {min(15, n_features)} Features:")
    ...
    ax.set_title(f'Top {n_features} Features - Random Forest\nDataset: {dataset_type.upper()}',
                 fontsize=14, fontweight='bold')
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Train Random Forest (Parquet + Path Independent)'
    )
    ...
    parser.add_argument('--max-depth', type=int, default=25,
                        help='Max tree depth (default: 25)')
