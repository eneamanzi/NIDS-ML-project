...
"""
Training Decision Tree per CICIoT2023.
Supporta training su dataset Originale o SMOTE.
"""
...
def load_data_flexible(train_path, base_data_dir):
    """
    Carica il Train set dal path specifico (Original o SMOTE)
    e Test/Validation dai path fissi CICIOT23 (sempre Originali).
    """
    print_header("LOADING DATASETS")
    
    # 1. Load Training Data (Path dinamico passato da argomento)
    ...
    # 2. Load Test & Validation Data (Path fissi CICIOT23)
    # Nota: Risaliamo alla cartella processed/CICIOT23 indipendentemente dal train
    # Assumiamo che base_data_dir punti a 'data/processed' o simile
    # Costruiamo il path assoluto per sicurezza
    
    # Se train_path è ".../SMOTE/train.pkl", dobbiamo puntare a ".../CICIOT23/test.pkl"
    # Usiamo una logica robusta basata sulla root del progetto
    # base_data_dir viene passato come '../data/processed/CICIOT23' dal default,
    # ma noi vogliamo assicurarci di puntare sempre a CICIOT23 per il test.
    
    # Risaliamo di due livelli dal file di train se siamo in SMOTE, o di uno se siamo in CICIOT23
    # Ma per semplicità, hardcodiamo il path relativo standard del progetto per test/val
    # assumendo di essere lanciati dalla root.
    
    test_path = "data/processed/CICIOT23/test_processed.pkl"
    val_path = "data/processed/CICIOT23/validation_processed.pkl"
    
    # Fallback: se lanciato da src/, aggiungi ../
    ...
    # Escludiamo le colonne non-feature
    exclude_cols = ['y_macro_encoded', 'y_specific']
...
def load_artifacts(base_dir="data/processed/CICIOT23"):
    """Carica LabelEncoder e Mapping."""
    # Gestione path se lanciato da src/
    if not os.path.exists(base_dir):
        base_dir = "../" + base_dir
        
    encoder_path = os.path.join(base_dir, "label_encoder.pkl")
    mapping_path = os.path.join(base_dir, "mapping_info.json")
    
    label_encoder = joblib.load(encoder_path)
    with open(mapping_path, 'r') as f:
        mapping_info = json.load(f)
        
    return label_encoder, mapping_info

# =============================================================================
# TRAINING & EVALUATION (Identici a prima)
# =============================================================================

def train_model(X_train, y_train, max_depth):
    print_header("TRAINING DECISION TREE")
    print(f"Hyperparameters: max_depth={max_depth}, class_weight='balanced'")
...
def evaluate_and_save(model, X_train, X_test, y_train, y_test, label_encoder, 
                      dataset_type, output_base_dir="docs", model_base_dir="models"):
    
    # Definisci directory specifiche in base al dataset_type
    algo_name = "DecisionTree"
    docs_dir = os.path.join(output_base_dir, algo_name, dataset_type)
    models_dir = os.path.join(model_base_dir, algo_name)
    
    os.makedirs(docs_dir, exist_ok=True)
    os.makedirs(models_dir, exist_ok=True)
    
    print_header(f"EVALUATION ({dataset_type.upper()})")
    ...
    plt.title(f"Confusion Matrix - {algo_name} ({dataset_type})")
    plt.savefig(os.path.join(docs_dir, "confusion_matrix.png"))
    plt.close()
    print(f"📊 Plots saved to: {docs_dir}")
    
    # Save Model
    model_filename = f"{algo_name}_{dataset_type}.pkl"
...
if __name__ == "__main__":
    ...
    # ARGOMENTI AGGIORNATI PER L'ORCHESTRATOR
    parser.add_argument('--dataset-type', type=str, required=True, 
                        choices=['original', 'smote'], help='Tag per output folder')
    parser.add_argument('--train-path', type=str, required=True, 
                        help='Path assoluto o relativo al file pkl di training')
    ...
    # 1. Carica Dati
    X_train, X_test, _, y_train, y_test, _, feature_cols = load_data_flexible(
        args.train_path, 
        "data/processed/CICIOT23" # Base per test set
    )
    
    # 2. Carica Encoder (serve per i nomi delle classi nei grafici)
    le, _ = load_artifacts()
    ...
    # 4. Evaluate & Save
    evaluate_and_save(model, X_train, X_test, y_train, y_test, le, args.dataset_type)